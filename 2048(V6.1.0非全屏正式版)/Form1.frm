...
Sub Image1_DblClick() '全屏与非全屏不同的过程
    ...
    moshi4.Top = MainForm.Top
    MainForm.Left = (Screen.Width - MainForm.Width - moshi4.Width) / 2
    moshi4.Left = MainForm.Left + MainForm.Width
    moshi4.Show 0
    Call jiance
End Sub
...
Sub Image3_DblClick() '全屏与非全屏不同的过程
    ...
    DeveloperMode.List1.Width = 360.8
    DeveloperMode.List2.Width = 264.8
    moshi6.Top = MainForm.Top
    MainForm.Left = (Screen.Width - MainForm.Width - moshi6.Width) / 2
    moshi6.Left = MainForm.Left + MainForm.Width
    moshi6.Show 0
    Call jiance
End Sub
...
Sub jiance() '全屏与非全屏不同的过程
    ...
    If v <> 5 Then
        ...
        If v = 4 Then
            moshi4.Caption = "2048(4*4) 模式:" + zfcjc(0, ms) + " 难度:" + zfcjc(nd, 0) + " 步数:" + Trim(Str(js - 1))
        ElseIf v = 6 Then
            moshi6.Caption = "2048(6*6) 模式:" + zfcjc(0, ms) + " 难度:" + zfcjc(nd, 0) + " 步数:" + Trim(Str(js - 1))
        End If
    End If
End Sub
...
Sub Form_Unload(Cancel As Integer) '非全屏状态独有
    myexit = MsgBox("即将退出", vbExclamation + vbDefaultButton2 + vbSystemModal, "退出之三")
